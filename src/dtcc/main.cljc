(ns dtcc.main
  "Kotodama WASM entrypoint for the Dtcc clean-room actor (L4) — Clojure port.
  ...
  facts (`dtcc.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Instrument" :plural "instruments" :id-prefix "dtcc_ins"
   ...
   {:entity "Position"   :plural "positions"   :id-prefix "dtcc_pos"
...
(defn emit-facts
  "EAVT facts for one record: {\"dtcc.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "dtcc-compat" :tier tier :entities entities} 200])
